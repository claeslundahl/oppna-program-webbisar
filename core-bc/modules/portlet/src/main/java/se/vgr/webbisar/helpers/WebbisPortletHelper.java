...
public class WebbisPortletHelper {
	...
	private ClassValidator webbisValidator = new ClassValidator( Webbis.class, ResourceBundle.getBundle("se.vgr.webbisar.types.ValidatorMessages", new Locale("sv")) );
	 ...
	 * @param imageSize
	 *            the pixel-size (of the longest side) of the images are scaled
	 *            to before saveing them on disk.
	 * @param imageQuality
	 *            the quality of the saved images. Should be a number between 0
	 *            and 100 where 100 gives the best quality.
	 */
	...
	/**
	 * This method will parse the multipart request and if the user has pressed
	 * cancel return without doing anything else. If the user has pressed submit
	 * it will save all uploaded images in the temp folder on disk. It will also
	 * save references to the images in the session with the prefix
	 * 'webbis.mainform.'.
	 * 
	 * @param request
				...
				if (!item.isFormField()) {
					...
					if (item.getSize() > 0) {
						if (!item.getContentType().startsWith("image")){
							throw new WebbisValidationException("Bildfilen måste vara av typen JPeg, GIF eller PNG");
						}
						...
						session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX
								+ item.getFieldName(), baseUrl + "temp/"
								+ session.getId() + "/" + filename);
						
						if (!isMainImageSet(request)) {
							setMainImage(Integer
									.parseInt(item.getFieldName().substring(
											item.getFieldName().length() - 1)),
									request);
						}
					}
	...
	/**
	 * This will remove all webbis-specific
	 * data from the session (i.e. everything that starts with 'webbis').
	 * 
	 * @param session
	 *            the portlet session
	 */
	...
	/**
	 * This method will save all main webbis form data in the session with the
	 * prefix 'webbis.mainform.'. It will also store an arraylist called
	 * 'webbis.availableImageIds' with all image id form fields (i.e. 'image1'
	...
	public void saveWebbisFormInSession(ActionRequest request) {
		...
		while (paramNames.hasMoreElements()) {
			...
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + nextName,
					request.getParameter(nextName));
		}
		...
		session.setAttribute(WEBBIS_SESSION_PREFIX + "availableImageIds",
				availableImageIds);
	}

	/**
	 * This method will remove all info corresponding to a certain image number
	 * from the session. Note: it will not remove any files from the file system 
	 * as this should be done when publishing the webbis. 
	 * 
	 * @param imageNumber - the image to remove
	 */
	public void removeImage(int imageNumber, ActionRequest request) {
		PortletSession session = request.getPortletSession(true);

		session.removeAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "image"
				+ imageNumber);
		session.removeAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "image"
				+ imageNumber + "-text");
		...
		if (session.getAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "main-image")
				.equals("image" + imageNumber)) {
			// set the first available image as the main image
			for (int i = 1; i <= 5; i++) {
				if (session.getAttribute(WEBBIS_MAINFORM_SESSION_PREFIX
						+ "image" + i) != null) {
					session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX
							+ "main-image", "image" + i);
					break;
	 ...
	 * 
	 * @param imageNumber the image to set as the main image
	 */
	public void setMainImage(int imageNumber, ActionRequest request) {
		PortletSession session = request.getPortletSession(true);
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "main-image",
				"image" + imageNumber);
	}
	...
	private boolean isMainImageSet(ActionRequest request) {
		...
		return ((mainImage != null) && (mainImage.toString()
				.startsWith("image")));
	...
	/**
	 * Creates a Webbis from the input request parameters. This method will also
	 * move the images from the temp area to their final destination before
	 * setting the links to the images in the Webbis. All links in the webbis 
	 * will be saved relative to the baseUrl/baseDir, i.e. baseUrl/baseDir will
	 * not be included. 
	 * 
	 * @param request
	 *            the ActionRequest
	...
	public Webbis createWebbis(ActionRequest request) throws WebbisValidationException {
		...
		int mainImage = (request.getParameter("main-image") == null || "".equals(request.getParameter("main-image"))) ? 1:Integer.parseInt(request.getParameter("main-image").substring(5));
		...
		if (id != null && !id.equals("")){
			webbis = new Webbis(Long.parseLong(id), name, getUserId(request), sex, birthTime,
					weight, length, hospital, home, parents, images, siblings, message, email, webpage);
		}
		else{
			webbis = new Webbis(name, getUserId(request), sex, birthTime,
						weight, length, hospital, home, parents, images, siblings, message, email, webpage);
		}
	...
	private String parseName(String name) {
		...
		if("skriv namn".equalsIgnoreCase(name)) name = "";		
		return name;
	}
	...
	/**
	 * This method parses an image from the input request data and adds it to the list of images. The 
	 * location of the image is set to a relative link (without baseDir/baseUrl).  
	 * 
	 * @param request
	 * @param images the list of images
	 * @param imageParamName e.g. 'image1'
	 */
	private void parseImage(ActionRequest request, List<Image> images,
	 ...
	 * @param request
	 * @param parents the list of parent names
	 * @param firstNameParam
	 * @param lastNameParam
	 */
	private void parseParentName(ActionRequest request, List<Name> parents,
			String firstNameParam, String lastNameParam) {
		...
		if("förnamn".equalsIgnoreCase(firstName)) firstName = "";
		if("efternamn".equalsIgnoreCase(lastName)) lastName = "";
	...
	/**
	 * This will store the user's webbisar in the session. This is used between showing
	 * the link-list of "my webbisar" and the edit webbis main page (when the
	 * user has selected a webbis to edit). The list is also used when deleting a webbis or 
	 * upon update to retrieve the "old" webbis' images.
	 * 
	 * @param request
	 * @param webbisar
	 *            a List<Webbis> of webbisar
	 */
	public void storeMyWebbisarInSession(PortletRequest request,
			List<Webbis> webbisar) {
		request.getPortletSession(true).setAttribute(
				WEBBIS_SESSION_PREFIX + "myWebbisar", webbisar);

	}

	/**
	 * This method will put each individual field from the webbis in the session
	 * with the prefix 'webbis.mainform'. This is used for easy showing on the
	 * main edit webbis page (which retrieves values from the session).
	 * 
	 * @param session
	 * @param Webbis
	 *            the webbis for which each individual value will be stored in
	 *            the session with the prefix 'webbis.mainform'
	 */
	public void putWebbisDataInSession(PortletSession session, Webbis webbis) {
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "webbisId",
				webbis.getId());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "webbisname",
				webbis.getName());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "gender", webbis
				.getSex().toString());

		BirthTime time = webbis.getBirthTime();
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "day", time
				.getDay());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "month", time
				.getMonth());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "year", time
				.getYear());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "hour", time
				.getHour());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "min", time
				.getMinutes());

		if (webbis.getWeight() != 0)
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "weight", webbis
				.getWeight());
		if (webbis.getLength() != 0)
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "length", webbis
				.getLength());

		List<Name> parents = webbis.getParents();
		int i = 1;
		for (Name name : parents) {
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "parentfname"
					+ i, name.getFirstName());
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "parentlname"
					+ i, name.getLastName());
			i++;
		}

		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "siblings",
				webbis.getSiblings());
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "hospital", webbis.getHospital() != null ? webbis.getHospital().getLongName() : "");
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "hometown",
				webbis.getHome());
		...
		for (Image image : images) {
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "image" + i,
					baseUrl + image.getLocation());
			session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX + "image" + i
					+ "-text", image.getText());
			i++;
		}
		session.setAttribute(WEBBIS_MAINFORM_SESSION_PREFIX +
				 "main-image", "image1"); // the first image in the list is always the main image.
	}
	...
	public PreviewWebbisBean createPreviewWebbisBean(ActionRequest request,
			Webbis webbis) {
